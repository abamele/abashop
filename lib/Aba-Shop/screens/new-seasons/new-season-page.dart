...
class NewSeasonPage extends StatefulWidget {
  ...
  @override
  State<NewSeasonPage> createState() => _NewSeasonPageState();
}

class _NewSeasonPageState extends State<NewSeasonPage> {
  @override
  Widget build(BuildContext context) {
    NewSeasonProvider seasonProvider = Provider.of(context);
    seasonProvider.getNewSeasonData();
    ...
    return Scaffold(
      ...
      drawer: NavberWidget(),
      body: seasonProvider.newSeasonDataList.isEmpty
          ? Center(
              child: Text("No Data"),
            )
          : GridView.builder(
              gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                  ...
                  crossAxisSpacing: 5,
                  mainAxisSpacing: 5),
              itemCount: seasonProvider.getNewSeasonProductDataList.length,
              itemBuilder: (context, index) {
                final data = seasonProvider.getNewSeasonProductDataList[index];
                return SingleNewSeason(productModel: data);
              }),
    );
  }
}
